 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ubtruefalse_test_helper extends question_test_helper {
     ...
     * @return qtype_ubtruefalse_question
     */
    public function make_truefalse_question_true() {
        question_bank::load_question_definition_classes('ubtruefalse');
        $tf = new qtype_ubtruefalse_question();
        test_question_maker::initialise_a_question($tf);
        $tf->name = 'UBTrue/false question';
        $tf->questiontext = 'The answer is true.';
        $tf->generalfeedback = 'You should have selected true.';
        $tf->penalty = 1;
        $tf->qtype = question_bank::get_qtype('ubtruefalse');
     ...
     * @return qtype_ubtruefalse_question
     */
    public function make_ubtruefalse_question_false() {
        question_bank::load_question_definition_classes('ubtruefalse');
        $tf = new qtype_ubtruefalse_question();
        test_question_maker::initialise_a_question($tf);
        $tf->name = 'UBTrue/false question';
